...
> 작성일: 2026-07-06 (2차 개정: Google/AWS → Whisper API/SenseVoice-Small 교체)
> 대상: Naver Clova Speech / Vito(리턴제로) STT / OpenAI Whisper API / SenseVoice-Small(로컬)
>
> 교체 사유: Google STT는 60초 초과 오디오에 GCS 업로드(서비스계정), AWS는 S3 버킷이 필수라
> 사이드 프로젝트 대비 세팅 비용이 과함. Whisper API(키 1개)로 "글로벌 범용" 축을 유지하고,
> SenseVoice-Small로 "무료 로컬 모델" 비교축을 추가해 선정 논거를 오히려 강화.

---

## 1. 프로젝트 개요

한국어 면접 음성 데이터셋(음성 + 정답 텍스트)을 이용해 STT 엔진 4종 — 국산 상용 API 2종(Clova, Vito),
글로벌 API 1종(Whisper), 무료 로컬 모델 1종(SenseVoice-Small) — 을 **동일 조건에서 벤치마크**하고,
그 결과를 근거로 **AI 면접 평가 서비스에 가장 적합한 STT를 선정**하는 사이드 프로젝트.
...
핵심 설계 원칙: **4개 엔진의 응답을 하나의 공통 스키마로 정규화**하는 Adapter 패턴.
후처리·평가 코드는 엔진에 독립적이 되어, 교체 비용이 어댑터 1개 구현으로 줄어든다.
(실제로 Google/AWS → Whisper/SenseVoice 교체가 어댑터 2개 교체만으로 완료됨 — 패턴의 효용 실증)
...
| 항목 | Clova Speech | Vito (리턴제로) | Whisper API (whisper-1) | SenseVoice-Small (로컬) |
|---|---|---|---|---|
| 한국어 특화 | ◎ (네이버 자체 학습) | ◎ (한국어 전용) | ○ (다국어 범용, 한국어 준수) | ○ (zh/yue/en/ja/ko 5개 언어 특화) |
| Word timestamp | ✅ 제공 | ✅ 제공 | ✅ (`timestamp_granularities=word`) | ⚠️ CTC alignment 기반 → 정밀도 실험 확인 |
| Sentence/segment timestamp | ✅ segment 단위 제공 | ✅ utterance 단위 | ✅ segment 단위 | △ (VAD segment 단위) |
| Confidence | ✅ segment 단위 | ✅ 제공 | ❌ word 단위 미제공 (segment avg_logprob만) | ❌ 미제공 |
| Speaker Diarization | ✅ (한국어 지원) | ✅ (한국어 지원) | ❌ | ❌ (FunASR 별도 모델 조합 필요) |
| 간투어("음","어") 전사 | ⚠️ 필터링 옵션 있음 → verbatim 설정 확인 | ✅ 간투어 보존 옵션 (`use_disfluency_filter=False`) | ⚠️ **간투어/반복 제거 경향 유명** → 실험 확인 | ⚠️ 미확인 → 실험 확인 |
| 감정/이벤트 태그 | ❌ | ❌ | ❌ | ✅ SER + AED 내장 (면접 평가에 보너스) |
| 실시간 스트리밍 | ✅ (gRPC) | ✅ (WebSocket) | ❌ (배치 전용) | ❌ (오프라인 배치) |
| 요청 방식 | 파일 업로드/URL | 파일 업로드 (비동기 폴링) | 파일 업로드 (동기, 25MB 제한) | 로컬 추론 (funasr) |
...
| 엔진 | 단가 (기준) | 100시간 | 1,000시간 | 10,000시간 |
|---|---|---|---|---|
| Clova Speech (장문) | 4원 / 15초 (≈960원/시간) | ≈ 9.6만 원 | ≈ 96만 원 | ≈ 960만 원 |
| Vito STT | ≈ 0.3원/초 (≈1,080원/시간, 월 무료량 별도) | ≈ 10.8만 원 | ≈ 108만 원 | ≈ 1,080만 원 |
| Whisper API | $0.006/분 (≈$0.36/시간 ≈ 500원/시간) | ≈ $36 (≈5만 원) | ≈ $360 (≈50만 원) | ≈ $3,600 (≈500만 원) |
| SenseVoice-Small | **0원** (로컬 추론) | 0원 + 서버비 | 0원 + 서버비 | 0원 + 서버비 |

관찰 포인트: **Whisper가 상용 API 중 최저가**(국산 API의 절반 수준, 단 환율 리스크).
SenseVoice는 API 비용이 0원이지만 추론 서버 운영비·장애 대응이 우리 몫이 된다 —
"무료 로컬로 이 품질이 나오는데 유료 API를 쓸 이유가 있는가"가 이 실험의 핵심 질문 중 하나.

### 5.3 구현 난이도 (Python/FastAPI 기준)

| 엔진 | 난이도 | 비고 |
|---|---|---|
| Whisper | ★☆☆ | API 키 1개 + REST 1회 호출. 가장 간단 |
| Vito | ★☆☆ | REST + JWT, 문서 간결. 비동기 폴링만 구현하면 됨 |
| Clova | ★★☆ | REST, Secret Key 방식. 장문 인식은 사실상 REST 한 번 호출 |
| SenseVoice | ★★☆ | 키는 불필요하나 torch/funasr 설치 + 모델(~1GB) 다운로드. 운영 시 추론 서버 필요 |

(참고: 당초 후보였던 Google STT/Amazon Transcribe는 ★★★ — 60초 초과 오디오에
GCS/S3 업로드가 강제되어 서비스계정·IAM·버킷 세팅이 필요. 이 마찰이 교체 사유였다)
...
│   │   └── sensevoice.py        # SenseVoice-Small (로컬, funasr)
│   ├── analysis/                # 후처리·음향분석
...
class STTAdapter(ABC):
    ...
    # transcribe()가 감싸서 latency를 일관되게 측정 (엔진별 측정 방식 차이 제거)
    # 동기 호출로 순차 실행 — 병렬 실행 시 latency 측정이 오염되기 때문
```
...
CREATE TABLE stt_results (
    ...
    api_name      TEXT NOT NULL,          -- clova / vito / whisper / sensevoice
    transcript    TEXT,
...
**시나리오 B — "Whisper를 선택해야 하는 이유"가 되는 경우**
> CER이 국산 API와 대등 + 상용 API 중 최저가(시간당 ≈500원) + 구현 최간단.
> → 단, **간투어/반복 제거 경향이 실험에서 확인되면 면접 평가용으로는 탈락 사유** (verbatim 20% 가중치에서 대폭 감점)

**시나리오 C — Vito가 다크호스가 되는 경우**
> CER이 Clova와 대등하면서 verbatim 전사가 더 충실한 경우 (통화음성 학습 특성상 간투어 보존에 유리할 가능성).

**시나리오 D — "SenseVoice로 충분하다"가 되는 경우**
> CER이 상용 API 대비 열세 폭이 작고(예: 2%p 이내) verbatim 보존이 확인되는 경우.
> → "API 비용 0원 + 감정 태그 보너스가 정확도 열세를 상회. 단 timestamp 정밀도와 추론 서버 운영 부담이 조건"
...
| `funasr` + `torch` | SenseVoice-Small 로컬 추론 | sensevoice 어댑터 |
